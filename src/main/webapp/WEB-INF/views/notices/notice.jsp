...
<div class="main-container">
    <div class="subTitle"><img src="../../../resources/img/triangle.png">공지사항</div>
    <c:if test="${sessionScope.id==1}">
        <button class="btn newContents" onclick="location.href='/notice/save-form'">새 글작성</button>
    </c:if>
    <table class="table1">
        <tr>
        ...
        <c:forEach items="${noticeList}" var="notices">
                <tr>
                    <td>${notices.id}</td>
                    <td onclick="location.href='/notice/findById?id='+${notices.id}" style="text-align: left; cursor: pointer">${notices.noticeTitle}</td>
                    <td>${notices.noticeWriter}</td>
                    <td>${notices.noticeCreatedDateTime}</td>
    ...
    <ul class="pagination justify-content-center">
        <c:choose>
            ...
            <c:otherwise>   <%--else와 동일 --%>
                <li class="page-item">
                    <button type="button" class="btn page-link" onclick="location.href='/notice/paging?page=${paging.page-1}'">이전페이지</button>
                </li>
            </c:otherwise>
        </c:choose>
        ...
        <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i" step="1">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item">
                        <button type="button" class="btn page-link" style="width: 37px" onclick="location.href='/notice/paging?page=${i}'">${i}</button>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:choose>
            ...
            <c:otherwise>
                <li class="page-item">
                    <button type="button" class="btn page-link" onclick="location.href='/notice/paging?page=${paging.page+1}'">다음페이지</button>
                </li>
            </c:otherwise>
        </c:choose>
    </ul>
